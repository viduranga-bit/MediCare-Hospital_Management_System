...
export default function LaboraristTable() {

  const [LaboraristList, setLaborarist] = useState([]);

  useEffect(() => {
    loadLaborarist();
  }, []);

  const loadLaborarist = async () => {
    const result = await axios
      .get("http://localhost:8080/api/v1/users/role/LABORARIST")
      .then((res) => {
        setLaborarist(res.data);
      });
  };
